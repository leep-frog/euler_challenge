...
func P874() *ecmodels.Problem {
	return ecmodels.IntInputNode(874, func(o command.Output, n int) {
		...
		o.Stdoutln(dp2(negativeSpace, negativeSpace, prime, n, 0))
...
func dp2(negativeSpace, at, numbersRemaining, n, currentScore int) int {
	...
	if at > 1 {
		best.Check(dp2(negativeSpace, at-1, numbersRemaining, n, currentScore))
	}

	// Check with adding one of these
	best.Check(p.Nth(n-1-at) + dp2(negativeSpace-at, maths.Min(at, negativeSpace-at), numbersRemaining-1, n, currentScore+p.Nth(n-1-at)))
