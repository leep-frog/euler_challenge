...
func P152() *ecmodels.Problem {
	return ecmodels.IntInputNode(152, func(o command.Output, n int) {
		...
		bfs.DFS([]*sumEquivalent{{nil, fraction.NewRational(0, 1)}}, ctx)
...
func (se *sumEquivalent) AdjacentStates(ctx *sumEquivalentContext) []*sumEquivalent {
	...
	for k := start; k <= ctx.n; k++ {
		r = append(r, &sumEquivalent{
			...
			se.sum.Plus(fraction.NewRational(1, k*k)),
		})
	}
	return r
}
